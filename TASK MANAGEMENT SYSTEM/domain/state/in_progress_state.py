...
class InProgressState(TaskState):
    def can_transition_to(self, new_status: TaskStatus) -> bool:
        return new_status in [TaskStatus.REVIEW, TaskStatus.CANCELLED]

    def perform_transition(self, task: Task, new_status: TaskStatus):
        if not self.can_transition_to(new_status):
            raise InvalidStateTransitionException(f"Cannot transition from IN_PROGRESS to {new_status.name}")
